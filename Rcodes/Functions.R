...
ineq_edag_i <- function(age, dx, lx, ex, ax,i){
  ...
  (rev(cumsum(rev(dx * ex_average*(-log(lx)^(i-1)))))/factorial(i-1))/lx
}
...
# estimating lambdas : intensity of lifesaving
...
# estimating lifespan inequality and entropy for those who are saved repeatedly

edagg<-function(lx,i){
  lx<- lx/100000
  sum((lx*(-log(lx)^i))/factorial(i), na.rm = T)
  }

H<-function(lx,i){
  lx<- lx/100000
  edagg<-sum((lx*(-log(lx)^i))/factorial(i), na.rm = T)
  H<-edagg/sum(lx)
  return(H)
}
